...
_antiunify_derivation_ files contain the specification and verification of the derivation relation obtained as the reflexive-transitive closure of the one-step application of the inference rules. The definitions for labels, protected variables, and lateral substitutions of a valid configuration are also presented. Lemmas about preservation properties and invariants regarding those definitions are proved in this file. Also, the history of the computation codified by the derivation of a valid configuration from an initial one is mechanically verified.
